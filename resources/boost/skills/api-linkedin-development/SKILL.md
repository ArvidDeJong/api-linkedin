...
- **Two LinkedIn products for any connect.** `openid` and `profile` come from "Sign In with LinkedIn using OpenID Connect", `w_member_social` from "Share on LinkedIn". LinkedIn refuses the whole authorization when one requested scope is not authorized, so an app with only one of the two cannot connect.
...
- To test the callback, sign in (the routes run through `auth` since 1.8) and put the state in the session first: `$this->actingAs($user)->withSession(['linkedin_oauth_state' => 'abc'])->get(route('linkedin.callback', ['code' => 'c', 'state' => 'abc']))->assertSessionHas('linkedin_status')`. With a `can:` ability in the middleware, the user has to pass that gate.
